package com.milk.tools.widget.autosviewpager;
...
public class AutoScrollViewPager extends ViewPager {
     ...
     *
     * deliver event to parent, add by fangzhu
     **/
     ...
     **/
    private int slideBorderMode = SLIDE_BORDER_MODE_TO_PARENT;
    /**
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (slideBorderMode == SLIDE_BORDER_MODE_TO_PARENT || slideBorderMode == SLIDE_BORDER_MODE_CYCLE) {
            ...
            if (ev.getAction() == MotionEvent.ACTION_DOWN) {
                //注意了 不要写 downX=touchX，当touchX改变downX也变了
                downX = ev.getX();
                downY = ev.getY();
            }
            int currentItem = getCurrentItem();
...
//            }
            ...
            if (slideBorderMode == SLIDE_BORDER_MODE_TO_PARENT) {
                ...
                //更具体的判断 第一页和最后一页
//                    if ((downX - touchX > 2 && currentItem == getAdapter().getCount() - 1) || (touchX - downX > 2 && currentItem == 0) || (Math.abs(downY - touchY) > Math.abs(downX - touchX))) {
//                        getParent().requestDisallowInterceptTouchEvent(false);
//                    } else
//                        getParent().requestDisallowInterceptTouchEvent(true);//X轴方向移动的距离大于Y轴方向
            } else {
                if (pageCount > 1) {
                    setCurrentItem(pageCount - currentItem - 1, isBorderAnimation);
                }
